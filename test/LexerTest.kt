...
inner class LexWordTest {
    ...
    fun `Word snake case`() {
        testInpOutp("asdf_abc") {
            it.error(errorWordUnderscoresOnlyAtStart)
        }
    }
    ...
    fun `Word correct capitalization 1`() {
        testInpOutp("Asdf.abc") {
            it.buildPunct(0, 8, statementFun, 1)
                .build(0, 0, 8, word)
        }
    }
    ...
    fun `Word correct capitalization 2`() {
        testInpOutp("asdf.abcd.zyui") {
            it.buildPunct(0, 14, statementFun, 1)
              .build(0, 0, 14, word)
        }
    }
    ...
    fun `Word correct capitalization 3`() {
        testInpOutp("Asdf.Abcd") {
            it.buildPunct(0, 9, statementFun, 1)
                .build(0, 0, 9, word)
        }
    }
    ...
    fun `Word incorrect capitalization`() {
        testInpOutp("asdf.Abcd") {
            it.error(errorWordCapitalizationOrder)
        }
    }
    ...
    fun `Word starts with underscore and lowercase letter`() {
        testInpOutp("_abc") {
            it.buildPunct(0, 4, statementFun, 1)
              .build(0, 0, 4, word)
        }
    }
    ...
    fun `Dotword & @-word`() {
        testInpOutp("@a123 .Abc ") {
            it.buildPunct(0, 11, statementFun, 2)
                .build(0, 0, 5, atWord)
                .build(0, 6, 4, dotWord)
        }
    }
...
inner class LexNumericTest {
    ...
    fun `Binary numeric 64-bit zero`() {
        testInpOutp("0b0") {
            it.buildPunct(0, 3, statementFun, 1)
              .build(0, 0, 3, intTok)
        }
    }
    ...
    fun `Binary numeric basic`() {
        testInpOutp("0b101") {
            it.buildPunct(0, 5, statementFun, 1)
              .build(5, 0, 5, intTok)
        }
    }
    ...
    fun `Binary numeric 64-bit positive`() {
        testInpOutp("0b0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0111") {
            it.buildPunct(0, 81, statementFun, 1)
              .build(7, 0, 81, intTok)
        }
    }
    ...
    fun `Binary numeric 65-bit error`() {
        testInpOutp("0b0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_01110") {
            it.error(errorNumericBinWidthExceeded)
        }
    }
    ...
    fun `Hex numeric 3`() {
        testInpOutp("0xFFFFFFFFFFFFFFFF") {
            it.buildPunct(0, 18, statementFun, 1)
    ...
    fun `Float numeric 5`() {
        testInpOutp("100500.123456") {
            it.buildPunct(0, 13, statementFun, 1)
    ...
    fun `Float numeric too big`() {
        testInpOutp("9007199254740993.0") {
            it.error(errorNumericFloatWidthExceeded)
        }
    }
    ...
    fun `Float numeric big exponent`() {
        testInpOutp("1005001234560000000000.0") {
            it.buildPunct(0, 24, statementFun, 1)
    ...
    fun `Int numeric 1`() {
        testInpOutp("3") {
            it.buildPunct(0, 1, statementFun, 1)
              .build(3, 0, 1, intTok)
        }
    }
    ...
    fun `Int numeric 2`() {
        testInpOutp("12") {
            it.buildPunct(0, 2, statementFun, 1)
              .build(12, 0, 2, intTok)
        }
    }
    ...
    fun `Int numeric 3`() {
        testInpOutp("0987_12") {
            it.buildPunct(0, 7, statementFun, 1)
    ...
    fun `Int numeric 4`() {
        testInpOutp("9_223_372_036_854_775_807") {
            it.buildPunct(0, 25, statementFun, 1)
    ...
    fun `Int numeric error 1`() {
        testInpOutp("3_") {
            it.error(errorNumericEndUnderscore)
        }
    }
    ...
    fun `Int numeric error 2`() {
        testInpOutp("9_223_372_036_854_775_808") {
            it.error(errorNumericIntWidthExceeded)
        }
    }
}
...
inner class LexStringTest {
    ...
    fun `String simple literal`() {
        testInpOutp("\"asdf\"") {
            it.buildPunct(0, 6, statementFun, 1)
    ...
    fun `String literal with escaped quote inside`() {
        testInpOutp("\"wasn\\\"t\" so sure") {
            it.buildPunct(0, 17, statementFun, 3)
    ...
    fun `String literal unclosed`() {
        testInpOutp("\"asdf") {
            it.error(errorPrematureEndOfInput)
        }
    }
...
//        testInpOutp(
//            "\"asdf foo\"\"\"",
//            it.buildPunct(0, 12, statementFun, 1)
...
inner class LexCommentTest {
    ...
    fun `Comment inline`() {
        testInpOutp("# this is a comment.# and #this too.# but not this") {
            it.buildPunct(22, 28, statementFun, 4)
...
inner class LexPunctuationTest {
    ...
    fun `Parens simple`() {
        testInpOutp("(car cdr)") {
            it.buildPunct(0, 9, statementFun, 3)
    ...
    fun `Parens nested`() {
        testInpOutp("(car (other car) cdr)") {
            it.buildPunct(0, 21, statementFun, 6)
    ...
    fun `Parens unclosed`() {
        testInpOutp("(car (other car) cdr") {
            it.buildPunct(0, 0, statementFun, 0)
    ...
    fun `Dot-brackets simple`() {
        testInpOutp("x.[y z]") {
            it.buildPunct(0, 7, statementFun, 4)
              .build(0, 0, 1, word)
...
bcjk
            ...
            it.buildPunct(1, 12, curlyBraces, 4)
    ...
    fun `Dollar 1`() {
        testInpOutp("""foo $ a b""") {
            it.buildPunct(0, 9, statementFun, 4)
              .build(0, 0, 3, word)
    ...
    @Test
    fun `Dollar 2`() {
        testInpOutp("""foo $ a b $ 6 5""") {
            it.buildPunct(0, 15, statementFun, 7)
              .build(0, 0, 3, word)
    ...
    @Test
    fun `Dollar 3`() {
        testInpOutp("""foo (bar $ 1 2 3)""") {
            it.buildPunct(0, 17, statementFun, 7)
              .build(0, 0, 3, word)
    ...
    fun `Punctuation scope inside statement 1`() {
        testInpOutp("""foo bar { asdf }""") {
            it.buildPunct(0, 16, statementFun, 5)
              .build(0, 0, 3, word)
              .build(0, 4, 3, word)
...
bcj
            ...
            it.buildPunct(0, 21, statementFun, 7)
              .build(0, 0, 3, word)
              .build(0, 4, 3, word)
...
bcjk ({a; b})
            ...
            it.buildPunct(1, 48, curlyBraces, 23)
...
inner class LexOperatorTest {
    ...
    fun `Operator simple 2`() {
        testInpOutp("+ - / * ** && || ^ <- -> := => >=. <=.") {
            it.buildPunct(0, 38, statementAssignment, 14)
              .build(OperatorToken(OperatorType.plus, 0, false).toInt(), 0, 1, operatorTok)
    ...
    fun `Operator expression`() {
        testInpOutp("a - b") {
            it.buildPunct(0, 5, statementFun, 3)
              .build(0, 0, 1, word)
              .build(OperatorToken(OperatorType.minus, 0, false).toInt(), 2, 1, operatorTok)
              .build(0, 4, 1, word)
        }
    ...
    fun `Operator assignment 1`() {
        testInpOutp("a += b") {
            it.buildPunct(0, 6, statementAssignment, 3)
              .build(0, 0, 1, word)
    ...
    fun `Operator assignment 2`() {
        testInpOutp("a ||= b") {
            it.buildPunct(0, 7, statementAssignment, 3)
              .build(0, 0, 1, word)
    ...
    fun `Operator assignment 3`() {
        testInpOutp("a ||=. b") {
            it.buildPunct(0, 8, statementAssignment, 3)
              .build(0, 0, 1, word)
    ...
    fun `Operator assignment in parens 1`() {
        testInpOutp("x += y + 5") {
            it.buildPunct(0, 10, statementAssignment, 5)
              .build(0, 0, 1, word)
              .build(OperatorToken(OperatorType.plus, 0, true).toInt(), 2, 2, operatorTok)
              .build(0, 5, 1, word)
    ...
    fun `Operator assignment in parens 2`() {
        testInpOutp("(x += y + 5)") {
            it.buildPunct(0, 12, statementAssignment, 6)
    ...
    fun `Operator assignment in parens 3`() {
        testInpOutp("((x += y + 5))") {
            it.buildPunct(0, 14, statementAssignment, 7)
    ...
    fun `Operator assignment multiple error 1`() {
        testInpOutp("x := y := 7") {
            it.buildPunct(0, 0, statementAssignment, 0)
              .build(0, 0, 1, word)
